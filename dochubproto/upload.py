...
class KeeperClient(object):
    ...
    def __init__(self, product_name, git_refs,
        ...
        super().__init__()
        self.log = get_logger(level=os.getenv('LOGLEVEL', 'WARNING'))
        self.product_name = product_name
        self.git_refs = git_refs
        if isinstance(git_refs, str):
            self.git_refs = [self.git_refs]
        else:
            self.git_refs = git_refs
    ...
    def surrogate_key(self):
        ...
        return self.build_info['surrogate_key']

    @property
    def s3_prefix(self):
        """Path prefix for the build."""
        return self.build_info['bucket_root_dir']
